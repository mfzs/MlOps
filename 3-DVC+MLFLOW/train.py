...
def parse_args():
    p = argparse.ArgumentParser("Simple MLflow demo (just prediction)")
    p.add_argument("--csv", default="data/data.csv", help="Path to CSV (default: data/data.csv)")
    p.add_argument("--target", default="quality", help="Target column name (default: quality)")
    p.add_argument("--experiment", default="just-prediction", help="MLflow experiment name")
...
def main():
    ...
    if not os.path.exists(args.csv):
        raise SystemExit(f"CSV not found: {args.csv}. Create or copy just_sample.csv next to this script.")
